...
k8s_yaml('./deployment/development/k8s/secrets.yaml')
k8s_yaml('./deployment/development/k8s/app-config.yaml')

### End of K8s Config ###
### RabbitMQ ###
k8s_yaml('./deployment/development/k8s/rabbitmq-deployment.yaml')
...
if os.name == 'nt':
 driver_compile_cmd = './deployment/development/docker/driver-build.bat'
...
docker_build_with_restart(
  'github.com/n-korel/nexus-drive-go/driver-service',
  '.',
  entrypoint=['/app/build/driver-service'],
  dockerfile='./deployment/development/docker/driver-service.Dockerfile',
  only=[
...
k8s_yaml('./deployment/development/k8s/driver-service-deployment.yaml')
...
if os.name == 'nt':
  payment_compile_cmd = './deployment/development/docker/payment-build.bat'
...
docker_build_with_restart(
  'github.com/n-korel/nexus-drive-go/payment-service',
  '.',
  entrypoint=['/app/build/payment-service'],
  dockerfile='./deployment/development/docker/payment-service.Dockerfile',
  only=[
...
k8s_yaml('./deployment/development/k8s/payment-service-deployment.yaml')
...
### Jaeger ###
k8s_yaml('./deployment/development/k8s/jaeger.yaml')

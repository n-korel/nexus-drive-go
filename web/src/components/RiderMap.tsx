...
export default function RiderMap({ onRouteSelected }: RiderMapProps) {
  ...
  const handleMapClick = async (e: L.LeafletMouseEvent) => {
    ...
    debounceTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        console.error("Failed to get ride preview:", err);
        alert("Failed to get ride preview. Please try again.");
      } finally {
  ...
  const handleStartTrip = async (fare: RouteFare) => {
    ...
    const payload: HTTPTripStartRequestPayload = {
      rideFareID: fare.id,
      userID: userID,
    };
    ...
    <div className="relative flex flex-col md:flex-row h-screen">
      <div className={`${destination ? "flex-[0.7]" : "flex-1"}`}>
        ...
        >
          ...
          {trip && trip.route && <RoutingControl route={trip.route} />}
